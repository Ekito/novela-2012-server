...
aj.removeValue=function(al){if(this.isRootNode()){return
}this.values.remove(al)
};
...
ao.canHandle=function(av){return ap(av)!=null
};
...
var ar=l.getLogger(aj);
var am="Sec-WebSocket-Protocol";
var an="Sec-WebSocket-Extensions";
var ao="Authorization";
var aq="WWW-Authenticate";
var ax="Set-Cookie";
var at="GET";
var ay="HTTP/1.1";
var aB=":";
var aC=" ";
var av="\r\n";
var aw=function(){ar.finest(aj,"<init>")
};
var az=function(aE,aG){ar.finest(aj,"sendCookieRequest with {0}",aG);
var aD=new XMLHttpRequest0();
var aF=aE._location.getHttpEquivalentScheme()+"://"+aE._location.getAuthority()+(aE._location._uri.path||"");
aF=aF.replace(/[\/]?$/,"/;api/set-cookies");
aD.open("POST",aF,true);
aD.setRequestHeader("Content-Type","text/plain; charset=utf-8");
aD.onreadystatechange=function(){};
aD.send(aG)
};
var ak=function(aI,aG,aE){var aF=[];
var aD=[];
aF.push("WebSocket-Protocol");
aD.push("");
aF.push(am);
aD.push(aG._protocol);
aF.push(an);
aD.push(L.KAAZING_SEC_EXTENSION_REVALIDATE);
aF.push(ao);
aD.push(aE);
var aH=al(aG._location,aF,aD);
aI._nextHandler.processTextMessage(aG,aH)
};
var al=function(aE,aI,aJ){ar.entering(aj,"encodeGetRequest");
var aL=[];
aL.push(at);
aL.push(aC);
var aK=[];
if(aE._uri.path!=undefined){aK.push(aE._uri.path)
}if(aE._uri.query!=undefined){aK.push("?");
aK.push(aE._uri.query)
}aL.push(aK.join(""));
aL.push(aC);
aL.push(ay);
aL.push(av);
...
var au=function(aD){var aE=aD.getString(Charset.UTF8);
return aE.split("\n")
};
var ap=function(aJ,aH,aD){aH.handshakePayload.putBuffer(aD);
if(aD.capacity>0){return
}aH.handshakePayload.flip();
var aP=au(aH.handshakePayload);
aH.handshakePayload.clear();
var aG="";
for(var aF=aP.length-1;
aF>=0;
aF--){if(aP[aF].indexOf("HTTP/1.1")==0){var aN=aP[aF].split(" ");
aG=aN[1];
break
}}if("101"==aG){var aL="";
for(var aF in aP){var aO=aP[aF];
if(aO!=null&&aO.indexOf(an)==0){aL=aO.substring(an.length+2)
}else{if(aO!=null&&aO.indexOf(am)==0){aH.protocol=aO.substring(am.length+2)
}else{if(aO!=null&&aO.indexOf(ax)==0){az(aH,aO.substring(ax.length+2))
}}}}if(aL.length>0){var aI=aL.split(", ");
for(var aF in aI){var aE=aI[aF].split("; ");
if(aE.length>1){var aM=aE[1];
aH._controlFrames[parseInt(aM,16)]=aE[0]
}}}return
}else{if("401"==aG){var aK="";
for(var aF in aP){if(aP[aF].indexOf(aq)==0){aK=aP[aF].substring(aq.length+2);
break
}}aJ._listener.authenticationRequested(aH,aH._location.toString(),aK)
}else{aJ._listener.connectionFailed(aH)
}}};
var aA=aw.prototype=new A();
aA.processConnect=function(aE,aD,aF){aE.handshakePayload=new ByteBuffer();
if(aF==null||aF.length==0){aF=L.KAAZING_EXTENDED_HANDSHAKE
}else{if(aF.indexOf(L.KAAZING_EXTENDED_HANDSHAKE)<0){aF+=","+L.KAAZING_EXTENDED_HANDSHAKE
}}this._nextHandler.processConnect(aE,aD,aF)
};
aA.processAuthorize=function(aE,aD){ak(this,aE,aD)
};
aA.handleConnectionOpened=function(aD,aE){ar.finest(aj,"handleConnectionOpened");
if(L.KAAZING_EXTENDED_HANDSHAKE==aE){ak(this,aD,null)
}else{aD._balanced=true;
this._listener.connectionOpened(aD,aD.protocol)
}};
aA.handleMessageReceived=function(aE,aD){ar.finest(aj,"handleMessageReceived",aD);
if(aE.readyState==1){aE._isEscape=false;
this._listener.messageReceived(aE,aD)
}else{ap(this,aE,aD)
}};
aA.setNextHandler=function(aD){this._nextHandler=aD;
var aF=this;
var aE={};
aE.connectionOpened=function(aG,aH){aF.handleConnectionOpened(aG,aH)
};
aE.messageReceived=function(aH,aG){aF.handleMessageReceived(aH,aG)
};
aE.connectionClosed=function(aG){aF._listener.connectionClosed(aG)
};
aE.connectionFailed=function(aG){aF._listener.connectionFailed(aG)
};
aE.authenticationRequested=function(aI,aG,aH){aF._listener.authenticationRequested(aI,aG,aH)
};
aE.redirected=function(aH,aG){aF._listener.redirected(aH,aG)
};
aD.setListener(aE)
};
aA.setListener=function(aD){this._listener=aD
};
return aw
})();
...
}if(ao!=null&&ao.canHandle(ap)){ao.handle(ap,function(aw){try{if(aw==null||aw.credentials==null){av.handleClearAuthenticationData(au);
av._listener.connectionFailed(au)
}else{au._challengeResponse=aw;
av._nextHandler.processAuthorize(au,aw.credentials)
}}catch(ax){av.handleClearAuthenticationData(au);
av._listener.connectionFailed(au)
}})
}else{this.handleClearAuthenticationData(au);
this._listener.connectionFailed(au)
}}};
...
aH.open("POST",aG._upstream,true);
...
var aA=l.getLogger(ak);
var ax="javascript:ws";
var an="javascript:wss";
var aw="javascript:wse";
var aD="javascript:wse+ssl";
var ay="flash:wse";
...
this._nativeHandler=az(this);
this._emulatedHandler=aB(this);
this._emulatedFlashHandler=ao(this);
this._rtmpFlashHandler=aE(this);
aA.finest(ak,"<init>");
av();
aG[ax]=new O("ws",this._nativeHandler,al);
aG[an]=new O("wss",this._nativeHandler,al);
aG[aw]=new O("ws",this._emulatedHandler,aj);
aG[aD]=new O("wss",this._emulatedHandler,aj);
aG[ay]=new O("ws",this._emulatedFlashHandler,aj);
aG[ar]=new O("wss",this._emulatedFlashHandler,aj);
aG[ap]=new O("ws",this._rtmpFlashHandler,aj);
aG[at]=new O("wss",this._rtmpFlashHandler,aj)
};
function av(){switch(browser){case"android":case"opera":am.ws=new Array(ax,aw);
am.wss=new Array(an,aD);
case"ie":var aH=navigator.appVersion;
if(aH.indexOf("MSIE 6.0")>=0||aH.indexOf("MSIE 7.0")>=0){am.ws=new Array(ax,ay,aw);
am.wss=new Array(an,ar,aD)
}else{am.ws=new Array(ax,aw,ay);
am.wss=new Array(an,aD,ar)
}case"firefox":case"chrome":case"safari":default:am.ws=new Array(ax,aw,ay);
am.wss=new Array(an,aD,ar)
}}function aq(aI){var aH={};
...
aH.connectionClosed=function(aJ){aI.handleConnectionClosed(aJ)
};
aH.connectionFailed=function(aJ){aI.handleConnectionFailed(aJ)
};
aH.authenticationRequested=function(aL,aJ,aK){};
aH.redirected=function(aK,aJ){};
return aH
}function az(aI){var aH=new aa();
var aJ=new E();
aH.setListener(aI._handlerListener);
aH.setNextHandler(aJ);
return aH
}function aB(aI){var aH=new aa();
var aJ=new J();
aH.setListener(aI._handlerListener);
aH.setNextHandler(aJ);
return aH
}function ao(aJ){var aH=new aa();
var aI=new T();
aH.setListener(aJ._handlerListener);
aH.setNextHandler(aI);
return aH
}function aE(aJ){var aH=new aa();
var aI=new U();
aH.setListener(aJ._handlerListener);
aH.setNextHandler(aI);
return aH
}var aC=function(aL,aK){var aO=aG[aK];
var aJ=aO._channelFactory;
var aH=aL._location;
var aN=aL._protocol;
var aM=aL._isBinary;
var aI=aJ.createChannel(aH,aN,aM);
aL._selectedChannel=aI;
aI.parent=aL;
aI._handler=aO._handler;
aI._handler.processConnect(aL._selectedChannel,aH,aN)
};
var aF=au.prototype;
aF.fallbackNext=function(aI){aA.finest(ak,"fallbackNext");
var aH=aI.getNextStrategy();
if(aH==null){this.doClose(aI)
}else{aC(aI,aH)
}};
aF.doOpen=function(aH){if(aH.readyState==0){aH.readyState=1;
aH._webSocket.handleOpen()
}};
aF.doClose=function(aH){if(aH.readyState==0||aH.readyState==1){aH.readyState=2;
aH._webSocket.handleClose()
}};
aF.processConnect=function(aM,aI,aO){aA.finest(ak,"connect",aM);
var aH=aM;
aA.finest("Current ready state = "+aH.readyState);
if(aH.readyState==1){aA.fine("Attempt to reconnect an existing open WebSocket to a different location");
throw new Error("Attempt to reconnect an existing open WebSocket to a different location")
}var aJ=aH._compositeScheme;
if(aJ!="ws"&&aJ!="wss"){var aN=aG[aJ];
if(aN==null){throw new Error("Invalid connection scheme: "+aJ)
}aA.finest("Turning off fallback since the URL is prefixed with java:");
aH._connectionStrategies.push(aJ)
}else{var aL=am[aJ];
if(aL!=null){for(var aK=0;
aK<aL.length;
aK++){aH._connectionStrategies.push(aL[aK])
}}else{throw new Error("Invalid connection scheme: "+aJ)
}}this.fallbackNext(aH)
};
aF.processTextMessage=function(aK,aJ){aA.finest(ak,"send",aJ);
var aI=aK;
if(aI.readyState!=1){aA.fine("Attempt to post message on unopened or closed web socket");
throw new Error("Attempt to post message on unopened or closed web socket")
}var aH=aI._selectedChannel;
aH._handler.processTextMessage(aH,aJ)
};
aF.processBinaryMessage=function(aK,aJ){aA.finest(ak,"send",aJ);
var aI=aK;
if(aI.readyState!=1){aA.fine("Attempt to post message on unopened or closed web socket");
throw new Error("Attempt to post message on unopened or closed web socket")
}var aH=aI._selectedChannel;
aH._handler.processBinaryMessage(aH,aJ)
};
aF.processClose=function(aJ){aA.finest(ak,"close");
var aI=aJ;
if(aI&&aI.readyState==2){aA.fine("WebSocket already closed");
throw new Error("WebSocket already closed")
}if(aI&&!aI._closing){aI._closing=true;
var aH=aI._selectedChannel;
aH._handler.processClose(aH)
}};
aF.setListener=function(aH){this._listener=aH
};
aF.handleConnectionOpened=function(aI,aJ){var aH=aI.parent;
this.doOpen(aH)
};
aF.handleMessageReceived=function(aJ,aH){var aI=aJ.parent;
aI._webSocket.handleMessage(aH)
};
aF.handleConnectionClosed=function(aI){var aH=aI.parent;
if(aH._closing){this.doClose(aH)
}else{if(aH.readyState==0&&!aI.authenticationReceived){this.fallbackNext(aH)
}else{this.doClose(aH)
}}};
aF.handleConnectionFailed=function(aI){var aH=aI.parent;
if(aH._closing){this.doClose(aH)
}else{if(aH.readyState==0&&!aI.authenticationReceived){this.fallbackNext(aH)
}else{this.doClose(aH)
}}};
